...
(a) Largest $p(C_k | x)$ is significantly less then 1. \\
(b) $p(x, C_k)$ have comparable values
...
(a) Inference stage : training data to learn a model for $p(C_k | x)$ \\
(b) Decision stage : use posterior probabilities to make optimal class assignments.
